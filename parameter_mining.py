...
def main(inputs):
    ...
    if not bool_binarysearch and not bool_quantitative: #Binary satisfaction with fixed granularity
        ...
        if bool_mono:  V1, counter = fun_BS.BinarySearchMonotonic(time_series_class1_train,  gran, par_bounds,fun.Approx_Val(1, [ ]) ,[], mono,  p_formula, sampling_freq, constant_definition)
        ...
        if bool_mono:   V2, counter = fun_BS.BinarySearchMonotonic(time_series_class2_train,  gran, par_bounds,fun.Approx_Val(2, [ ]) ,[], mono,  p_formula, sampling_freq, constant_definition)
        ...
        if len(traces)>2 :
            if bool_mono:  V3, counter = fun_BS.BinarySearchMonotonic(time_series_class3_train,  gran, par_bounds, fun.Approx_Val(3, [ ]) ,[], mono,  p_formula, sampling_freq, constant_definition)
